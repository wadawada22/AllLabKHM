...
const vertexShaderSourceBox = `
    ...
    void main(void) {
        ...
        // нормалізуємо нормалі
        vNormal = normalize(mat3(uNormalMatrix) * aNormal);

        // розр. напрям світла
        vec3 worldPosition = (uMatrix * aPosition).xyz;
        vLightDirection = normalize(uLightPosition - worldPosition);
        vViewDirection = normalize(uViewPosition - worldPosition);
    }
`;
...
const fragmentShaderSourceBox = `
    ...
    void main(void) {
        ...
        // Відблиски
        float gleamIntensity = 1.5; // Boost intensity of the reflection
        vec3 gleamColor = vec3(1.0); // White gleam color
        vec3 reflection = gleamIntensity * spec * gleamColor;

        // загальний
        vec3 finalColor = (ambient + diffuse + specular + reflection) * vColor.rgb;
...
function initGL() {
    ...
    gl.uniform1f(shininessLoc, 1.0); 
}

//допоміжна функція для встановлення та зв'яз. буферів
